...
function ImageCarousel({ images, title }) {
  ...
  useEffect(() => {
    setCurrentIndex(0)
    // Reset carousel DOM error state
    if (carouselRef.current) {
      carouselRef.current.querySelectorAll('img').forEach(img => {
        img.style.display = 'block';
      });
      carouselRef.current.querySelectorAll('.carousel-placeholder').forEach(el => {
        el.remove();
      });
    }
  }, [images])
        ...
        <img
          ...
          onError={(e) => {
            ...
            if (index === currentIndex) {
              ...
              if (!e.target.parentElement.querySelector('.carousel-placeholder')) {
